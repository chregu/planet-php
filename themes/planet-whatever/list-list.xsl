...
><xsl:import href="main-list.xsl"/>
	...
	blog<xsl:value-of select="blog_id"/><xsl:text> </xsl:text>
     <xsl:choose>
        <xsl:when test="position() mod 2 = 1">box listView</xsl:when>
        <xsl:otherwise>box listView odd</xsl:otherwise>
        </xsl:choose>
	</xsl:attribute>
        <div class="col1">
        ...
        <div class="col2">
               ...
               <a class="listMore"  id="more{id}" onclick="moreInfoStart('{blog_id}','{id}'); return false;" href="#{blog_id}_{id}">i</a>
                    <xsl:if test="haswerbung/text() = 1"><span title="Paid/Sponsored Content - Publireportage - Werbung"><xsl:value-of select="$sponsoredEntries"/><xsl:text> </xsl:text> </span> </xsl:if>
