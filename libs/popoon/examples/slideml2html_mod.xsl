    ...
    <xsl:template match="/s:slideset">
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            <head>
				...
				<link rel="stylesheet" type="text/css" href="pres/slideml.css"></link>
            </head>
            <body>
				<xsl:if test="s:slide[position() = $page - 1]">
				<a href="./page{$page - 1}.html">prev</a>
				</xsl:if>
<xsl:text> </xsl:text>
				<xsl:if test="s:slide[position() = $page + 1]"><a href="./page{$page + 1}.html">next</a></xsl:if>
